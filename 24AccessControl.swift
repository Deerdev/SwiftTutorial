 ...
 *      文件私有访问(file-private): 限制实体只能被所定义的 文件内部 访问。当需要把这些细节被整个文件使用的时候，使用文件私有访问隐藏了一些特定功能的实现细节。
...
/// 默认访问级别 internal (Default Access Levels)

/// 单元测试访问级别 “Access Levels for Unit Test Targets”
...
/// 常量 变量 属性 下标 “Constants, Variables, Properties, and Subscripts”
